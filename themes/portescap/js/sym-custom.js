...
jQuery(document).ready(function() {
	...
	if(jQuery('body').hasClass('node-type-product-group') || jQuery('body').hasClass('node-type-related-product-group')) {
		jQuery('.line-item .spec-relocate, .line-item .dxf-relocate, .line-item .3d-relocate').each(function() {
...
function changeVideo(el, title) {
	jQuery('.field-name-field-video-embed-code').find('iframe').remove();
	jQuery('#content h1, #content h2').each(function() {
...
jQuery(document).ready(function() {
	...
	jQuery('.video-thumb').click(function() {
		...
		var theTitle = jQuery(this).find('h4').text();
		changeVideo(theVideo, theTitle);
	});
